\section{Chapter 6 --- Bootstrapping Experiments: Ladder, Yields, and the Five-Pairing Confirmation}\label{sec:chapter-6-bootstrapping-experiments-ladd}
...
are as follows. SFT runs at learning rate 1$\times$10⁻⁴ for 3 epochs with cosine decay.
Rejection-sampling SFT (RSFT) continues the SFT adapter at 5$\times$10⁻⁵ for 2 epochs. DPO continues
the RSFT adapter at 5$\times$10⁻⁶ for 1 epoch with sigmoid preference loss and $\beta$ = 0.1
...
Training on the self-harvest (RSFT: continuing the SFT adapter at 5$\times$10⁻⁵ for 2 epochs on
...
(68 steps) at 5$\times$10⁻⁶ with $\beta$ = 0.1.
...
p = 3$\times$10⁻⁷, 5$\times$10⁻⁷, 2$\times$10⁻⁵), so the development-set student--teacher gap cannot be an artifact
