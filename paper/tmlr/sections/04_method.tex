...
\subsubsection{Streaming ingestion and release selection}
...
\subsubsection{Role-aware entity resolution}
...
\subsubsection{Fact extraction and provenance}
...
\subsubsection{Graph construction and validation}
...
\subsection{Closed typed algebra}
\label{sec:algebra}
...
abstention object.  The specialised parser uses Qwen3-8B \citep{yang2025qwen3} with 4-bit QLoRA
\citep{dettmers2023qlora}.  LoRA rank is 64, scaling $\alpha=128$, dropout is 0.05, and all
target modules are adapted.  Training runs for two epochs with per-device batch size 2,
gradient accumulation 8 (effective batch 16), learning rate $10^{-4}$, cosine decay, 3\%
warm-up, a 4096-token cutoff, and bfloat16 computation.  The held-out synthetic loss is reported
only as an optimisation diagnostic because the export contains duplicates and two exact
train/validation overlaps.

\subsection{Schema-grounded understanding and conditional planning}
...
\subsection{Grounding and graph execution}
\label{sec:grounding}
...
\subsection{Evidence, release checks, and answer cards}
...
\subsection{Bounded replanning and abstention}
\label{sec:repair}
