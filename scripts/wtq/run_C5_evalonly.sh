...
test -s outputs/wtq_C_v5/checkpoint-1000/adapter_config.json
CUDA_VISIBLE_DEVICES=3 .venv/bin/vllm serve Qwen/Qwen3-8B --port 8001 --host 127.0.0.1 \
  --chat-template-content-format string --max-model-len 8192 --gpu-memory-utilization 0.50 \
  --default-chat-template-kwargs '{"enable_thinking": false}' \
  --enable-lora --max-lora-rank 64 --lora-modules wtq_C5=outputs/wtq_C_v5/checkpoint-1000 \
  > /tmp/claude-1847/8001_C5e.log 2>&1 &
SPID=$!
